...
import com.Ani.AndroidGame.Common.ButtonConstants;
...
public class InputGameInterface extends BaseObject {
    ...
    public void update(float timeDelta, BaseObject parent) {
		InputSystem input = gameObjectRegistry.inputSystem;
		...
		final float gameTime = gameObjectRegistry.timeSystem.getGameTime();
		...
		if (mUseOnScreenControls) {
			...
			if (sliderTouch != null) {
				...
				final float magnitude = magnitudeRamp * UtilityFunction.sign(offset) * SLIDER_FILTER * mMovementSensitivity;
				sliderOffset = magnitudeRamp * UtilityFunction.sign(offset);
			...
			if (trackball.getLastPressedTime() > Math.max(leftPressedTime, rightPressedTime)) {
				...
				if (gameTime - trackball.getLastPressedTime() < ROLL_TIMEOUT) {
					...
					if (x != 0.0f) {
						int sign = UtilityFunction.sign(x);
						x = x - (sign * ROLL_DECAY * timeDelta);
						if (UtilityFunction.sign(x) != sign) {
							x = 0.0f;
						}
					}
					
					if (y != 0.0f) {
						int sign = UtilityFunction.sign(y);
